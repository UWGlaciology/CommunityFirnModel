...
- *example.json* Fixed a stale `DFresample` value (`"1M"`) that raised `ValueError: Invalid frequency: 1M` on dataframe-input runs with recent pandas, which drops the `'M'` offset alias. Updated to `"1ME"`, matching example_df.json/example_csv.json.
