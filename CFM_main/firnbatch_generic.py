...
the CFM is called using main.py and a .json. 
...
(you need GNU parallel installed)
...
from spin_up_generator_CFM import makeSpinFiles
import xarray as xr
import pandas as pd
...
def run_CFM(LLpair):
    ...
    RCdrive = 'drive_name:' # make sure it has : at the end
    runtype = 'local'
    datatype = 'MERRA'
    movefiles = False 
    ...
    if writeCFMinputs:
        ...
        CFM_df.to_pickle('CFMpickle/CFM_df_{}_{}_{}_{}_{}'.format(lat_int,lon_int,timeres,Tinterp,dwriter))
    ...
    firnS = FirnDensitySpin(configName,climateTS = Cd)
    firnS.time_evolve()
    telap = (time.time()-tnow)/60
    print('spin done, {} minutes'.format(telap))

    firn = FirnDensityNoSpin(configName,climateTS = Cd)
    firn.time_evolve()
    telap = (time.time()-tnow)/60
