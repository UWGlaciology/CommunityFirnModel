...
# def HL_dynamic(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    ...
    def Helsen_2008(self):
        ...
        A_instant = self.bdotSec * self.steps * BDOT_TO_A
        A_mean = self.bdot_mean * self.steps * BDOT_TO_A

        if self.bdot_type == 'instant':
            dr_dt = (RHO_I - self.rho) * A_instant * (76.138 - 0.28965 * self.Ts) * 8.36 * (K_TO_C - self.Tz) ** -2.061
        elif self.bdot_type == 'mean':
            dr_dt = (RHO_I - self.rho) * A_mean * (76.138 - 0.28965 * self.Ts) * 8.36 * (K_TO_C - self.Tz) ** -2.061
