...
class Diffusion:
    def __init__(self, z, stp, gridLen, init_Tz, init_del_z):
        ...
        # Create red noise surface isotope vector and vertical profile isotope vector
        ar1 = 0.5   # red noise memory coefficient
        std_rednoise = 2    # red noise standard deviation
        self.del_s = std_rednoise*np.random.randn(stp)     # white noise shifted to -50 average
               
        for x in range(2,stp):
            self.del_s[x] = self.del_s[x-1]*ar1 + np.random.randn() -50  # create red noise from white
