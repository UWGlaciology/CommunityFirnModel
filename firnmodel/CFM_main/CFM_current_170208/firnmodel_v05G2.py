...
#             print A
            ...
            drho_dt[rho>=rho1] = dr_dt[rho>=rho1]
            # print rho[5:15]
            # print drho_dt[5:15]/917.0 
            if ii<=5:
                print 'drho_dt1_6', drho_dt[0:6]                   
...
#                     drho_dt[j] = dr_dt/sPerYear
            ...
            if D0>0.59: #Transition has to be less than D=0.6
                D0=0.59
            # ind4 = np.argmin(D[D>=D0])
            # print 'ind4', ind4
            # print 'Dind4', D[ind4-1:ind4+3]
            Dms = D0 + 0.009 #D0 + epsilon factor, maximum is 0.599
            # epsilon=0.009
            Dmsrho=Dms*rhoi2 #density of change
            print 'D0', D0
            ...
            # ind1=ind4+1 #first index where the density is greater than or equal to the transition
            print 'ind1', ind1
            print 'rhoLoopStart', rho[ind1-1:ind1+2]
            Dm = D[ind1] #actual transition density. Use this so that the transition falls at a node and avoids the sinularity
            Dmrho=Dm*rhoi2
            print 'Dm', Dm
            ...
            # A = 7.89e3*np.exp(-Qgj/(R*Tz))*1.0e-3 #A given in MPa^-3 s^-1, Goujon uses bar as pressure unit
            A = 7.89e3*np.exp(-Qgj/(R*Tz))*1.e-3 #A given in MPa^-3 s^-1, Goujon uses bar as pressure unit
            ccc = 15.5 # no units given
            Z0g=(110.2*D0**3.-148.594*D0**2.+87.6166*D0-17.) #from Anais' code           
            print 'Z0g=', Z0g
            lp=(D/D0)**(1.0/3.0)
            ...
            # gamma_An=(5.3*A[ind1] * (Dms**2*Dms)**(1.0/3.0) * (a[ind1]/np.pi)**(1.0/2.0) * (sigmastar[ind1]/3.0)**n) / ((sigma_bar[ind1]/(Dms**2))*(1-(5.0/3.0*Dms))); 
            # Dm=Dm+epsilon
            gamma_An=(5.3*A[ind1] * (Dm**2*(D0))**(1.0/3.0) * (a[ind1]/np.pi)**(1.0/2.0) * (sigmastar[ind1]/3.0)**3.0) / ((sigma[ind1]/(Dm**2))*(1-(5.0/3.0*Dm))); 
            ...
            # raw_input('press enter')    
            dDdt[D<Dm]=gamma*(sigma_bar[D<Dm])*(1.0-(5.0/3.0)*D[D<Dm])/((D[D<Dm])**2.0)
            dDdt[D>=Dm]=nn*5.3*A[D>=Dm]* (((D[D>=Dm]**2.0)*D0)**(1/3.)) * (a[D>=Dm]/np.pi)**(1.0/2.0) * (sigmastar[D>=Dm]/3.0)**3.0     
        
            # if ii<5:
            #     print dDdt[ind1-3:ind1+4]
            #     raw_input()
            ...
            # if ii!=0 and dDdt[ind1-1]<=dDdt[ind1] and gamma!=gamma_old2: #and dDdt_old[ind1]!=dDdt_old[ind1]:
            # if dDdt[ind1-1]<=dDdt[ind1]: #and gamma!=gamma_old2: #and dDdt_old[ind1]!=dDdt_old[ind1]:
            while dDdt[ind1-1]<dDdt[ind1]:
                # print 'g1'
                gamma=gamma*(gam_div)
                # print "g1", gamma
                # raw_input()
                dDdt[D<Dm]=gamma*(sigma_bar[D<Dm])*(1.0-(5.0/3.0)*D[D<Dm])/((D[D<Dm])**2.0)
                dDdt[D>=Dm]=nn*5.3*A[D>=Dm]* (((D[D>=Dm]**2.0)*D0)**(1/3.)) * (a[D>=Dm]/np.pi)**(1.0/2.0) * (sigmastar[D>=Dm]/3.0)**3.0           # # then iterate to find the maximum value of gamma that will make a continuous drho/dt
            while dDdt[ind1-1]>=dDdt[ind1]:
                gam_div=1.001
                gamma=gamma/gam_div
                # print "g2", gamma
            ...
            # print 'gamma diff', gamma-gamma_An
            # if ii<5:
            print 'gamma', gamma
            print 'dDdt1_6', dDdt[0:6]
            print 'dDdt', dDdt[ind1-2:ind1+3]
                # raw_input()            
            # raw_input('press enter')
            ...
            if ii<=5:
                print 'drho_dt1_6', drho_dt[0:6]
                print 'drho_dt', drho_dt[ind1-2:ind1+3]
...
#             print sigma_b
        ...
        rho = rho + dt*drho_dt
        if ii<=5:
            print 'rho_new_16', rho[0:6]
            try:
                print 'rho_new', rho[ind1-2:ind1+3]
            except:
                print "no ind"
