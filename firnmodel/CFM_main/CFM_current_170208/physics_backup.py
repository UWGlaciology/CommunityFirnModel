...
class FirnPhysics:
    ...
    def Goujon_2003(self):
        ...
        ind1        = np.argmax(D[D <= Dms]) #first index where the density is greater than or equal to the transition
        print 'ind1', ind1
        if self.iii
        Dm          = D[ind1] #actual transition relative density. Use this so that the transition falls at a node
        Dmsrho      = Dm * rhoi2 #density of first node, zone 2
