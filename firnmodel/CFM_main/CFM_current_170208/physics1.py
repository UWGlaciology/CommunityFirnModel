...
# if you want to add physics that require more parameters, you need to alter this list and change it for all of the physics

def HL_dynamic(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
# def HL_dynamic(iii, steps, gridLen, bdotSec, Tz, rho):
    '''
    ...
    A = bdotSec[iii] * steps * BDOT_TO_A
    drho_dt = np.zeros(gridLen)
    dr_dt = drho_dt

    
#     print len(np.exp(-Q1 / (R * Tz[rho < RHO_1])))
#     print len((RHO_I_MGM - rho[rho < RHO_1] / 1000))
# #     print RHO_1
#     print len(np.power(A, aHL) * 1000 / S_PER_YEAR)
...
def HL_Sigfus(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''
...
def Li_2004(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''
...
def Li_2011(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''
...
def Arthern_2010S(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''
...
def Arthern_2010T(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''

    :param gridLen: grid length
    :param Tz:
    :param rho:
    :param sigma:
    :param r2:
...
def Helsen_2008(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''
...
def Simonsen_2013(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''
...
def Ligtenberg_2011(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''
...
def Barnola_1991(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''
...
def Morris_HL_2013(iii, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, sigma, dt, Ts, r2, physGrain):
    '''
    ...
    drho_dt[rho >= RHO_1] = k2 * np.exp(-Q2 / (R * Tz[rho >= RHO_1])) * (RHO_I_MGM - rho[rho >= RHO_1] / 1000) * np.power(A[iii], bHL) * 1000 / S_PER_YEAR
...
def grainGrowth(Tz, Ts, iii, dt, r2, calcGrainSize):
    '''
...
#     r2_time = np.concatenate(([t * iii + 1], r2))
    ...
    if calcGrainSize:												# uses surface temperature to get an initial grain size
        r2 = np.concatenate(([-2.42e-9 * Ts[iii] + 9.46e-7], r2[:-1]))
    else:
