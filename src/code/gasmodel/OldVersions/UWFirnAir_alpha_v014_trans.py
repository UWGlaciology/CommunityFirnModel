...
def FirnAir_TR(z_edges_vec,z_P_vec,nt,dt,Gamma_P,bc_u,bc_d,phi_0,rhoHL,deepnodes,R,nz_P,nz_fv,por_op,gas,p_a,por_tot,por_cl):
    ...
    f_depth=np.loadtxt(os.path.join(DataPath,'depthS.csv'),delimiter=',',skiprows=1)
    f_depth=f_depth[:,1:]

    f_density=np.loadtxt(os.path.join(DataPath,'densityS.csv'),delimiter=',',skiprows=1)
...
def diffusivity(rho_co, por_co, por_tot, por_cl, por_op, rhoprof = None): #rhoprof is  density profile
    ...
    diffu_full[:]=1.e-25 #Use these lines for advection testing.
    d_eddy[:]=1.e-25
...
if __name__ == "__main__":
    ...
    if transdiffu == 'on':
        ...
        phi=FirnAir_SS(z_edges_vec,z_P_vec,nt,dt,Gamma_P,bc_u,bc_d,phi_0,rhoHL,deepnodes, R,nz_P,nz_fv,por_op,gas,Accu_0,T,p_a)
