...
def runModel(configName,spin):
    ...
    ResultsPlace=os.path.join(spot,c['resultsFolder'])
...
#             F1=1.25 # Simonsen's recommended (email correspondence)
        ...
        elif c['physRho'] =='Ligtenberg2011': #b_dot is in kg/year.
            ar1=0.07
            ar2=0.03
            Ec=60.0e3
            Eg=42.4e3
...
#         if spin:
# #             print np.min(rho_old-rho)
# #             print np.max(rho_old-rho)
#             
#             if np.max(np.abs(rho_old[rho<815.]-rho[rho<815.]))<=0.01:
#                 logging.info('spin up reached steady state at %s' % mtime)
#                 break
