...
class FirnDensitySpin:
    def __init__(self, configName):
        ...
        input_temp, input_year_temp = read_temp(self.c['InputFileNameTemp'],self.c['InputFileFolder'])
        input_bdot, input_year_bdot = read_bdot(self.c['InputFileNamebdot'],self.c['InputFileFolder'])

        # load in model parameters
        self.bdot0 = input_bdot[0]
        self.temp0 = input_temp[0]
#         gridLen, dx, dt, t, stp, Ts, T_mean, bdotSec, rhos0 = self.define_parameters()
        self.gridLen, self.dx, self.dt, self.t, self.stp, self.Ts, self.T_mean, self.bdotSec, self.rhos0 = self.define_parameters()
        # set up model grid
        ...
        self.sigma = self.mass * self.dx * GRAVITY #is this definitely dx?
    ...
    def time_evolve(self):
        ...
        # load in model parameters
#         dx, dt, t, stp, Ts, T_mean, bdotSec, rhos0 = self.define_parameters() #2/12/16 - artifact?
        steps = 1 / self.t #2/12/16?
        if not self.c['physGrain']:
            r2_time = None

        for iter in xrange(self.stp):
            ...
            parameters = {
                'HLdynamic',      [steps, self.gridlen, bdotSec, self.diffu.Tz, self.rho],
                'HLSigfus',       [steps, self.gridlen, bdotSec, self.diffu.Tz, self.rho, sigma],
                'Barnola1991',    [steps, self.gridlen, bdotSec, self.diffu.Tz, self.rho, sigma],
                'Li2004',         [steps, self.gridlen, bdotSec, T_mean, self.rho],
                'Li2011',         [steps, self.gridlen, bdotSec, self.bdot_mean, self.c['bdot_type'], self.diffu.Tz, T_mean, self.rho],
                'Ligtenberg2011', [steps, self.gridlen, bdotSec, self.bdot_mean, self.c['bdot_type'], self.diffu.Tz, T_mean, self.rho],
                'Arthern2010S',   [steps, self.gridlen, bdotSec, self.bdot_mean, self.c['bdot_type'], self.diffu.Tz, T_mean, self.rho],
                'Simonsen2013',   [steps, self.gridlen, bdotSec, self.bdot_mean, self.c['bdot_type'], self.diffu.Tz, T_mean, self.rho],
                'Morris2013',     [steps, self.gridlen, self.diffu.Tz, dt, self.rho, True, iter],
                'Helsen2008',     [steps, bdotSec, self.c['bdot_type'], self.bdot_mean, self.diffu.Tz, Ts, self.rho],
                'Arthern2010T',   [self.gridlen, self.diffu.Tz, self.rho, self.sigma, self.r2, self.c['physGrain']],
                'Spencer2001',    [],
                'Goujon2003',     [],
            }
            try:
...
#         zz = np.min(self.z[self.rho > 850.0]) #use this for setting a self-determining spin up length; does not work 2/12/15
#         years = int(zz / self.bdot0) #couple this with previous line
        years = self.c['yearSpin']
