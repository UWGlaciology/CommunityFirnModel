...
class FirnPhysics:
    def __init__(self, iter, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho, Ts, sigma,r2, physGrain):
        self.iter=iter
        self.steps=steps
        self.gridLen=gridLen
        self.bdotSec=bdotSec
        self.Tz=Tz
        self.rho=rho


    def HL_dynamic(self):
    # def HL_dynamic(iter, steps, gridLen, bdotSec, Tz, rho):
        '''
        ...
        A = bdotSec[self.iter] * self.steps * BDOT_TO_A
    ...
    #     print len(np.power(A, aHL) * 1000 / S_PER_YEAR)
        ...
        dr_dt[rho < RHO_1]   = k1 * np.exp(-Q1 / (R * self.Tz[self.rho < RHO_1])) * (RHO_I_MGM - self.rho[self.rho < RHO_1] / 1000) * np.power(A, aHL) * 1000 / S_PER_YEAR
        drho_dt[self.rho < RHO_1] = dr_dt[self.rho < RHO_1]


        dr_dt[rho >= RHO_1]   = k2 * np.exp(-Q2 / (R * self.Tz[self.rho >= RHO_1])) * (RHO_I_MGM - self.rho[self.rho >= RHO_1] / 1000) * np.power(A, bHL) * 1000 / S_PER_YEAR
        drho_dt[self.rho >= RHO_1] = dr_dt[self.rho >= RHO_1]

        return drho_dt

    def HL_Sigfus(iter, steps, gridLen, bdotSec, Tz, rho, sigma):
        '''
    ...
    def Li_2004(iter, steps, gridLen, bdotSec, T_mean, rho):
        '''
    ...
    def Li_2011(iter, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho):
        '''
    ...
    def Arthern_2010S(iter, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho):
        '''
    ...
    def Arthern_2010T(iter, gridLen, Tz, rho, sigma, r2, physGrain):
        '''
    ...
    def Helsen_2008(iter, steps, bdotSec, bdot_mean, Tz, Ts, rho, bdot_type):
        '''
    ...
    def Simonsen_2013(iter, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho):
        '''
    ...
    def Ligtenberg_2011(iter, steps, gridLen, bdotSec, bdot_mean, bdot_type, Tz, T_mean, rho):
        '''
    ...
    def Barnola_1991(iter, steps, gridLen, bdotSec, Tz, rho, sigma):
        '''
    ...
    def Morris_HL_2013(iter, steps, spin, gridLen, Tz, dt, rho):
        '''
    ...
    def grainGrowth(Tz, Ts, iter, dt, r2, calcGrainSize):
        '''
    ...
    #     r2_time = np.concatenate(([t * iter + 1], r2))
        ...
        if calcGrainSize:												# uses surface temperature to get an initial grain size
            r2 = np.concatenate(([-2.42e-9 * Ts[iter] + 9.46e-7], r2[:-1]))
        else:
