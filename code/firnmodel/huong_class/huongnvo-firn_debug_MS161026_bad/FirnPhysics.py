...
class FirnPhysics:
    ...
    # def HL_dynamic(iter, steps, gridLen, bdotSec, Tz, rho):
        ...
        A = self.bdotSec[self.iter] * self.steps * BDOT_TO_A
    ...
    def Goujon_2003():
        ...
        pass
class GrainGrowth:
    """docstring for GrainGrowth"""
    def __init__(self, Tz, Ts, iter, dt, r2, calcGrainSize):
        # super(GrainGrowth, self).__init__()
        self.Tz = Tz
        self.Ts = Ts
        self.iter = iter
        self.dt = dt
        self.r2 = r2
        self.calcGrainSize = calcGrainSize
    ...
    #     print len(r2)
    #     print len(dr2_dt)
        self.r2 = self.r2 + dr2_dt * self.dt
    #     r2_time = np.concatenate(([t * iter + 1], r2))

        if self.calcGrainSize:												# uses surface temperature to get an initial grain size
            self.r2 = np.concatenate(([-2.42e-9 * self.Ts[self.iter] + 9.46e-7], self.r2[:-1]))
        else:
            self.r2 = np.concatenate(([(0.1e-3) ** 2], self.r2[:-1]))
