...
class FirnDensitySpin:
    ...
    def time_evolve(self):
        ...
        fphysics = {
            ...
            # 'HLSigfus':        FirnPhysics.HL_Sigfus,
            # 'Barnola1991':     FirnPhysics.Barnola_1991,
            # 'Li2004':          FirnPhysics.Li_2004,
            # 'Li2011':          FirnPhysics.Li_2011,
            # 'Ligtenberg2011':  FirnPhysics.Ligtenberg_2011,
            # 'Arthern2010S':    FirnPhysics.Arthern_2010S,
            # 'Simonsen2013':    FirnPhysics.Simonsen_2013,
            # 'Morris2013':      FirnPhysics.Morris_HL_2013,
            # 'Helsen2008':      FirnPhysics.Helsen_2008,
            # 'Arthern2010T':    FirnPhysics.Arthern_2010T,
            # 'Spencer2001':     FirnPhysics.Spencer_2001,
            # 'Goujon2003':      FirnPhysics.Goujon_2003,
        }
...
#             print 'z110=',len(self.z)
            ...
            parameters = {
                'PhysParams':     [iter, steps, self.gridLen, bdotSec, self.bdot_mean, self.c['bdot_type'], self.diffu.Tz, T_mean, self.rho, Ts, self.sigma,self.r2, self.c['physGrain']],
                'GGParams':       [self.diffu.Tz, Ts, iter, dt, self.r2, self.c['calcGrainSize']],
            ...
            # parameters = {
            #     'iter':     [iter, steps, self.gridLen, bdotSec, self.bdot_mean, self.c['bdot_type'], self.diffu.Tz, T_mean, self.rho, Ts, self.sigma,self.r2, self.c['physGrain']]
            #     # 'HLdynamic':      [iter, steps, self.gridLen, bdotSec, self.diffu.Tz, self.rho],
            #     # 'HLSigfus':       [iter, steps, self.gridLen, bdotSec, self.diffu.Tz, self.rho, self.sigma],
            #     # 'Barnola1991':    [iter, steps, self.gridLen, bdotSec, self.diffu.Tz, self.rho, self.sigma],
            #     # 'Li2004':         [iter, steps, self.gridLen, bdotSec, T_mean, self.rho],
            ...
            try:
                ...
                mrun=FirnPhysics(*parameters['PhysParams'])
                # print self.c['physRho']
                # drho_dt = mrun.fphysics[self.c['physRho']]
                drho_dt = mrun.HL_dynamic()
            except KeyError:
                default()
            ...
            # update grain radius
            if self.c['physGrain']:
                mrun=GrainGrowth(*parameters['GGParams'])
                self.r2 = mrun.grainGrowth()
