...
def file_impt(fileName):
	in_data = np.genfromtxt(fileName, delimiter=',')
	yrs = in_data[0,:]
	data = in_data[1,:]
...
def linear_interp(stps, years_t, temp, years_b, bDot, f_bdot, f_temp):
	max_t = max(years_t)
	max_b = max(years_b)
	min_t = min(years_t)
	min_b = min(years_b)
	...
	name_t = 'interp_' + f_temp
	name_b = 'interp_' + f_bdot
...
if __name__ == '__main__':
    ...
    #python data_interpMax.py bdotfilename.csv tempfilename.csv
    
    f_bdot=sys.argv[1]
    f_temp=sys.argv[2]
    
    bdot_yrs,bdot_data=file_impt(f_bdot)
    temp_yrs,temp_data=file_impt(f_temp)
    
    stps = 10.0
    
    name_t, name_b = linear_interp(stps, temp_yrs, temp_data, bdot_yrs, bdot_data, f_bdot, f_temp)
