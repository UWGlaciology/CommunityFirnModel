...
### Master copy 3/1/16: added nitrogen isotopes.
...
#         elif not spin and (t*ii)%1 == 0:
            ...
            if np.size(rho_time_write)==0:
                rho_time_write=rho_time
                Tz_time_write=Tz_time
                age_time_write=age_time
                z_time_write=z_time
                comprate_time_write=comprate_time
                Dcon_time_write=Dcon_time
                stress_time_write=stress_time                
                if c['physGrain']:
                    r2_time_write = r2_time
                if c["physRho"]=='Morris2014':
                    Hx_time_write = Hx_time
                Clim_time_write = Clim_time
                bdot_time_write = bdot_time
                
                nnn=type(rho_time_write)
                           
            else:
                rho_time_write=np.vstack([rho_time_write,rho_time])
                Tz_time_write=np.vstack([Tz_time_write,Tz_time])
                age_time_write=np.vstack([age_time_write,age_time])
                z_time_write=np.vstack([z_time_write,z_time])
                comprate_time_write=np.vstack([comprate_time_write,comprate_time])
                Dcon_time_write=np.vstack([Dcon_time_write,Dcon_time])
                stress_time_write=np.vstack([stress_time_write,stress_time])                
                if c['physGrain']:
                    r2_time_write = np.vstack([r2_time_write,r2_time])
                if c["physRho"]=='Morris2014':
                    Hx_time_write = np.vstack([Hx_time_write,Hx_time])
                Clim_time_write = np.vstack([Clim_time_write,Clim_time])
                bdot_time_write = np.vstack([bdot_time_write,bdot_time])
                           
            if rho_time_write.nbytes>=50.0e6:
                print 'writing to file',ii             
                with open(densityPath, "a") as f:
                    writer = csv.writer(f)
                    writer.writerows(rho_time_write)
                with open(tempPath, "a") as f:
                    writer = csv.writer(f)
                    writer.writerows(Tz_time_write)
                with open(agePath, "a") as f:
                    writer = csv.writer(f)
                    writer.writerows(age_time_write)
                with open(depthPath, "a") as f:
                    writer = csv.writer(f)
                    writer.writerows(z_time_write)
                with open(compratePath, "a") as f:
                    writer = csv.writer(f)
                    writer.writerows(comprate_time_write)
                with open(DconPath, "a") as f:
                    writer = csv.writer(f)
                    writer.writerows(Dcon_time_write)
                with open(stressPath, "a") as f:
                    writer = csv.writer(f)
                    writer.writerows(stress_time_write)
                with open(ClimPath, "a") as f:
                    writer = csv.writer(f)
                    writer.writerows(Clim_time_write)
                with open(bdotPath, "a") as f:
                    writer = csv.writer(f)
                    writer.writerows(bdot_time_write)
                if c['physGrain']:
                    with open(r2Path, "a") as f:
                        writer = csv.writer(f)
                        writer.writerows(r2_time_write)
                if c["physRho"]=='Morris2014':                
                    with open(HxPath, "a") as f:
                        writer = csv.writer(f)
                        writer.writerows(Hx_time_write)
